...
//            }
        ...
        $scope.saveAs = function ($event) {
            var skinName = prompt('Введите имя скина:');
            if (skinName !== null) {
                var $src = $($event.target || $event.srcElement);
                Spinner.show($src);
...
define('controllers/blog_modules/categories/form',['./../../module', 'underscore'], function (module, _) {
    ...
    module.controller('BlogModuleCategoriesFormController', ['$scope', '$routeParams', '$location', 'Rest', 'Spinner', function ($scope, $routeParams, $location, Rest, Spinner) {
        ...
        $scope.delete = function ($event) {
            var $src = $($event.target || $event.srcElement);
            Spinner.show($src);
            if (!confirm('Удалить модуль "' + $scope.module_data.module_name + '"?')) {
                Spinner.hide($src);
                return;
            }
        ...
        return {
            Transitions: function () {
                return [
                    ...
                    { id: 'fade',           name: 'Затухание' },
                    { id: 'slideTop',       name: 'Слайд сверху' },
                    { id: 'slideRight',     name: 'Слайд справа' },
                    { id: 'slideBottom',    name: 'Слайд снизу' },
                    { id: 'slideLeft',      name: 'Слайд слева' },
                    { id: 'carouselRight',  name: 'Карусель справа налево' },
                    { id: 'carouselLeft',   name: 'Карусель слева направо' }
                ];
            }
        };
    });
                    ...
                    $scope.edit = function() {
                        $modal.open({
                            ...
                            controller: function($scope, ngModel, $modalInstance) {
                                ...
                                // preview text
                                $scope.dummyText = 'Так выглядит Русский шрифт. Here is the English font.';
                                $scope.isEditable = false;
                                $scope.editText = function() {
                                    $scope.isEditable = !$scope.isEditable;
                                };
                                $scope.getEditBtnText = function() {
                                    return $scope.isEditable ? "Сохранить" : "Изменить";
                                };
                ...
                $scope.selectImage = function() {
                    if (Journal2Config['oc2']) {
                        ...
                        $('#dialog').dialog({
                            title: 'image manager',
                            bgiframe: false,
                            width: typeof Journal2ImageManagerWidth === 'undefined' ? 800 : Journal2ImageManagerWidth,
                            height: typeof Journal2ImageManagerHeight === 'undefined' ? 600 : Journal2ImageManagerHeight,
                            resizable: false,
                            modal: false,
                            close: function() {
    ...
    module
        .directive('categorySearch', ['Search', function (Search) {
            return {
                ...
                controller: function ($scope) {
                    $scope.options = Search.generateOptions('index.php?route=module/journal2/rest/catalog/categories', 'category_id', 'Выберите категорию...');
                }
            };
        }]);
});
    ...
    module
        .directive('informationSearch', ['Search', function (Search) {
            return {
                ...
                templateUrl: 'view/journal2/tpl/directives/search/search.html?ver=' + Journal2Config.version,
                controller: function ($scope) {
                    $scope.options = Search.generateOptions('index.php?route=module/journal2/rest/catalog/information', 'information_id', 'Выберите информацию...');
                }
            };
        }]);
});
    ...
    module
        .directive('manufacturerSearch', ['Search', function (Search) {
            return {
                ...
                templateUrl: 'view/journal2/tpl/directives/search/search.html?ver=' + Journal2Config.version,
                controller: function ($scope) {
                    $scope.options = Search.generateOptions('index.php?route=module/journal2/rest/catalog/manufacturers', 'manufacturer_id', 'Выберите бренд...');
                }
            };
        }]);
});
    ...
    module
        .directive('productSearch', ['Search', function (Search) {
            return {
                ...
                templateUrl: 'view/journal2/tpl/directives/search/search.html?ver=' + Journal2Config.version,
                controller: function ($scope) {
                    $scope.options = Search.generateOptions('index.php?route=module/journal2/rest/catalog/products', 'product_id', 'Выберите продукт...');
                }
            };
        }]);
});
    ...
    module
        .directive('blogCategorySearch', ['Search', function (Search) {
            return {
                ...
                templateUrl: 'view/journal2/tpl/directives/search/search.html?ver=' + Journal2Config.version,
                controller: function ($scope) {
                    $scope.options = Search.generateOptions('index.php?route=module/journal2/rest/blog/categories', 'category_id', 'Выберите категорию...', 'categories');
                }
            };
        }]);
});
    ...
    module
        .directive('blogPostSearch', ['Search', function (Search) {
            return {
                ...
                templateUrl: 'view/journal2/tpl/directives/search/search.html?ver=' + Journal2Config.version,
                controller: function ($scope) {
                    $scope.options = Search.generateOptions('index.php?route=module/journal2/rest/blog/posts', 'post_id', 'Выберите статью...', 'posts');
                }
            };
        }]);
});
                    ...
                    $scope.edit = function () {
                        $modal.open({
                            ...
                            controller: function ($scope, $rootScope, $modalInstance, ngModel, editor) {
                                ...
                                $scope.title = 'Настройка Границ';

                                if ($scope.editor === 'hide-style') {
                                    $scope.title = 'Настройка Радиуса';
                                }
                    ...
                    $http
                        ...
                        .success(function(response){
                            ...
                            if (typeof(response) !== 'object') {
                                deferred.reject('Нет доступа ' + url);
                            }
