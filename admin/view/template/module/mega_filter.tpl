...
var MFP = {
	...
	init: function( modules, firstModule ) {
		...
		jQuery('#add-new-module').click(function(){
			self._row = $('#nav-tabs > li:not([id="module-add"]):last');
	...
	save: function( txt, fn ) {
		...
		function save( idx ) {
			$$.post( '<?php echo $action_save_data; ?>'.replace(/&amp;/g,'&') + MF_AJAX_PARAMS + '&mf_idx=' + idx + '&mf_count=' + pages + '&mf_id=' + id, getData( idx ), function(){
				if( idx < pages ) {
					save( idx + 1 );
				} else {
					if( fn() === true ) {
				...
				.append(jQuery('<div id="tab-' + self._row + '-settings" class="tab-pane active">')
					.append(jQuery('<table class="form">')
						...
						.append(jQuery('<tr>')
							...
							.append(jQuery('<td data-name="store-id">'))
						)
						.append(jQuery('<tr>')
							.append('<td><?php echo $entry_position; ?></td>')
							.append(jQuery('<td data-name="position">'))
						)
						.append(jQuery('<tr>')
							.append('<td><?php echo $entry_status; ?></td>')
							.append(jQuery('<td data-name="status">'))
						)
						.append(jQuery('<tr>')
							.append('<td><?php echo $entry_sort_order; ?></td>')
							.append(jQuery('<td data-name="sort-order">'))
						)
					)
				)
				/**
	...
	_initModule: function( $module, data ) {
		...
		$module.find('[data-load-type]').bind('click',function(){
			...
			jQuery.get('<?php echo $action_ldv; ?>'.replace(/&amp;/g,'&')+MF_AJAX_PARAMS, { 'name' : '<?php echo $_name; ?>_module[' + type + ']', 'type' : type, 'idx' : row }, function( response ){
	...
	createCategories: function( data ) {
		var self	= this,
			...
			box		= jQuery('<div id="tab-' + row + '-categories" class="tab-pane">')
				.append(jQuery('<div class="btn-group btn-group-sm" style="margin: 10px 0">')
					.append(jQuery('<button type="button" disabled="disabled" class="btn btn-default" style="color: #000; opacity: 1;">Add new type:</button>'))
					.append(jQuery('<button type="button" data-type="related" class="btn btn-primary"><i class="glyphicon glyphicon-plus-sign"></i> <?php echo $text_related; ?></button>'))
				),
		...
		box.find('[data-type]').click(function(){
			add( jQuery(this).attr('data-type') );
		...
		function add( type, data ) {
			...
			tbody.append(jQuery('<tr>')
				...
				.append(jQuery('<td>')
					.append( self.createField( 'checkbox', null, '1', {
						'name'		: name + '[auto_levels]',
						'checked'	: typeof data == 'undefined' || ! data.auto_levels ? false : true
					}))
				)
			);
			...
			tbody.append(jQuery('<tr>')
				...
				.append(jQuery('<td>')
					.append( self.createField( 'checkbox', null, '1', {
						'name'		: name + '[collapsed]',
						'checked'	: typeof data == 'undefined' || ! data.collapsed ? false : true
					}))
				)
			);
			...
			tbody.append(jQuery('<tr>')
				...
				.append(jQuery('<td>')
					.append( self.createField( 'checkbox', null, '1', {
						'name'		: name + '[show_button]',
						'checked'	: typeof data == 'undefined' || ! data.show_button ? false : true
					}))
				)
			);
			...
			cnt.append(jQuery('<table class="table">')
				.append(jQuery('<thead>')
					.append(jQuery('<tr>')
	...
	createContainer: function( id, type ) {
		...
		return jQuery('<div id="tab-' + self._row + '-' + id + '" class="tab-pane">')
			.append('<a href="#" data-load-type="' + type + '" class="btn btn-xs btn-info" style="float:right; margin: 4px 0;"><i class="glyphicon glyphicon-repeat"></i> Load Default Values</a><div class="clearfix"></div>')
			.append( cnt );
	},
