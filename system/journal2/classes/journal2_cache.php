...
class Journal2Cache {
    ...
    /* длительность кеширования (3600 равно 1 час) */
    const EXPIRE = 3600;
    ...
    public function getJournalAssetsFileName($extension) {
        return "journal-skin-{$this->skin_id}-" . self::$device ."-{$this->language_id}-" . ".{$extension}";
    }
    ...
    public function set($property, $value) {
        if (!$this->developer_mode && self::canCache()) {
            $file_name = self::getCachePath() . (time() + self::EXPIRE) . "_j2_{$property}_sk{$this->skin_id}_s{$this->store_id}_l{$this->language_id}_c{$this->currency_id}_u{$this->logged_in}_" . self::$device ."_{$this->price}_" . "_{$this->hostname}.cache.html";
    ...
    private function getFileName($property) {
        $file_name = self::getCachePath() . "*_j2_{$property}_sk{$this->skin_id}_s{$this->store_id}_l{$this->language_id}_c{$this->currency_id}_u{$this->logged_in}_" . self::$device ."_{$this->price}_" . "_{$this->hostname}.cache.html";
