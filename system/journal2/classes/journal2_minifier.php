...
class Journal2Minifier {
    ...
    private function printStyle($href) {
        return '<link rel="stylesheet" href="' . Journal2Utils::staticAsset($href, $this->minify_css) . '"/>' . PHP_EOL;
    }

    private function printScript($src, $def = '') {
        $def = rtrim($def) . ' ';
        return '<script type="text/javascript"' . $def . 'src="' . Journal2Utils::staticAsset($src, $this->minify_css) .  '"></script>' . PHP_EOL;
    }
